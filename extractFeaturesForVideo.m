function extractFeaturesForVideo(filepath, filename, finalTime, a, t)

    for i=80000:80000
        initialTime = finalTime-i;

        % Read the file
        myPrintEventsForVideo(filepath, filename, initialTime, finalTime, a,t);

        % Load results
        load(fullfile(filepath, strcat(filename, '_', sprintf('%09d', finalTime), '_', sprintf('%09d', initialTime))), 'posLastEventPosition', 'posTimeStamp', 'posVectorOfTimeStamps', 'firstTimeStamp');
        
        % Create the frame    
        posLastEventPosition = posLastEventPosition-1;
        save(fullfile(filepath, strcat(filename,'TimeStamps_', sprintf('%09d', finalTime), '_', sprintf('%09d', initialTime))), 'posLastEventPosition', 'posTimeStamp', 'posVectorOfTimeStamps', 'firstTimeStamp'); % Save orientation
    end 
    ...
    for i=80000:80000
        ...
        % Load results
        load(fullfile(filepath, strcat(filename, '_', sprintf('%09d', finalTime), '_', sprintf('%09d', initialTime))), 'events', 'sizex', 'sizey'); 
    ...
    %     figure, imagesc(frame), axis image    
        ...
        save(fullfile(filepath, strcat(filename,'Orientation_', sprintf('%09d', finalTime), '_', sprintf('%09d', initialTime))), 'orientationFrame', 'filteredOrientationFrame', 'positionVectorOfTimeStamps', 'hog'); % Save orientation
    end
...
%     thresholdDistance = 0.5;
    ...
    for i=80000:80000
        ...
        % Load results
        load(fullfile(filepath, strcat(filename, '_', sprintf('%09d', finalTime), '_', sprintf('%09d', initialTime))), 'events', 'sizex', 'sizey', 'posLastEventPosition');
        events.x = events.x+1; events.y = events.y+1;
        ...
        % Create the frame
        posLastEventPosition = posLastEventPosition-1;
        save(fullfile(filepath, strcat(filename,'TempGradient_', sprintf('%09d', finalTime), '_', sprintf('%09d', initialTime))), 'posLastEventPosition', 'Ox', 'Oy', 'flowTimeStamp', 'posFrame');
    end
    ...
    for i=80000:80000        
        ...
        load(fullfile(filepath, strcat(filename,'TimeStamps_', sprintf('%09d', finalTime), '_', sprintf('%09d', initialTime))), 'posTimeStamp');
        ...
        save(fullfile(filepath, strcat(filename,'Baseline_', sprintf('%09d', finalTime), '_', sprintf('%09d', initialTime))), 'dvsContour', 'Inorm', 'edgeCanny'); % Save orientation
    end
    ...
    for i=80000:80000        
        ...
        % Load the last map with the last time stamp locations
        load(fullfile(filepath, strcat(filename,'TimeStamps_', sprintf('%09d', finalTime), '_', sprintf('%09d', initialTime))), 'posTimeStamp');
	    load(fullfile(filepath, strcat(filename, '_', sprintf('%09d', finalTime), '_', sprintf('%09d', initialTime))), 'sizex', 'sizey'); 
...
%                 gaborFilterResponses_phase(:,:,k) = angle(EO{s,o});
       ...
       save(fullfile(filepath, strcat(filename,'Texture_', sprintf('%09d', finalTime), '_', sprintf('%09d', initialTime))), 'gaborFilterResponses', 'gaborFilterResponsesMax', 'textureSegmentation'); % Save orientation
    end
