...
* MODIFIED by Ching L Teo (06/27/14)
...
void mexFunction( int nl, mxArray *pl[], int nr, const mxArray *pr[] )
{
  ...
  uint16 *eBinsFG = (uint16*) mxGetData(mxGetField(model,0,"eBinsFG"));
  uint32 *eBndsFG = (uint32*) mxGetData(mxGetField(model,0,"eBndsFG"));
  uint16 *eBinsBG = (uint16*) mxGetData(mxGetField(model,0,"eBinsBG"));
  uint32 *eBndsBG = (uint32*) mxGetData(mxGetField(model,0,"eBndsBG"));
  ...
  float *Efg = (float*) mxGetData(pl[1]);
  pl[2] = mxCreateNumericArray(3,outDims,mxSINGLE_CLASS,mxREAL);
  float *Ebg = (float*) mxGetData(pl[2]);

  pl[3] = mxCreateNumericArray(3,indDims,mxUINT32_CLASS,mxREAL);
  uint32 *ind = (uint32*) mxGetData(pl[3]);
  ...
  for( int c0=0; c0<gtWidth/stride; c0++ ) {
	...
	float *E2 = Efg + (r*stride) + (c*stride)*h2;
        float *E3 = Ebg + (r*stride) + (c*stride)*h2;
        int b0=eBnds[k], b1=eBnds[k+1]; //if(b0==b1) continue;
	//printf("%d,%d,%d\n",k,b0,b1);
        // label edges appropriately
	if (b0!=b1) for( int b=b0; b<b1; b++ ) E1[eids[eBins[b]]]++;
	int bFg0=eBndsFG[k], bFg1=eBndsFG[k+1];
        if (bFg0!=bFg1) for( int b=bFg0; b<bFg1; b++ ) E2[eids[eBinsFG[b]]]++;
	int bBg0=eBndsBG[k], bBg1=eBndsBG[k+1];
	if (bBg0!=bBg1) for( int b=bBg0; b<bBg1; b++ ) E3[eids[eBinsBG[b]]]++;
