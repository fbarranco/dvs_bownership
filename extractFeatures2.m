function extractFeatures2(filepath, filename, finalTime)
...
%     end
%	 clear positionVectorOfTimeStamps;
%	 clear hog;
%	 clear orientationFrame;
%	 clear filteredOrientationFrame;
% 
% 
...
%     end
%	 clear posLastEventPosition;
%	 clear posTimeStamp;
%	 clear posVectorOfTimeStamps;
%	 clear firstTimeStamp;
...
%        myPrintEvents(filepath, filename, initialTime, finalTime);
        ...
        [Ox, Oy, flowTimeStamp] = visualFlow(events, blockSize, thresholdActivity, thresholdDistance, maxspeed, sizex, sizey);        

        % Create the frame
        posLastEventPosition = posLastEventPosition-1;
        save(fullfile(filepath, strcat(filename,'TempGradient_', num2str(finalTime), '_', num2str(initialTime))), 'posLastEventPosition', 'Ox', 'Oy', 'flowTimeStamp'); 
    end
